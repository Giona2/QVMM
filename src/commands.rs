...
pub fn new() {
	...
	fs::create_dir(data::get_data_dir() + "/" + name)
		.expect("Failed to create directory for the VM");
	fs::create_dir(data::get_data_dir() + "/" + name + "/disk")
		.expect("Failed to create disk directory");
	Command::new("qemu-img")
		.args(["create", "-f", "qcow2", &(data::get_data_dir() + "/" + name + "/disk/0_current.qcow2"), storage_amount])
		.status().unwrap();
	fs::write(data::get_data_dir() + "/" + name + "/config.yaml", format!("ram_amount: \"{ram_amount}\""))
		.expect("Failed to create config.yaml file");
}

pub fn list() {
	for vm_name in fs::read_dir(data::get_data_dir()).unwrap() {
...
pub fn run(vm_name: Option<&str>) {
	...
	match selected_disk.to_lowercase().as_str() {
		"y" => {
			...
			let yaml_config = &YamlLoader::load_from_str(&fs::read_to_string(data::get_data_dir() + "/" + selected_vm + "/" + "config.yaml").unwrap()).unwrap()[0];
			Command::new("qemu-system-x86_64").args([ "-enable-kvm",
					"-hda", &(data::get_data_dir() + "/" + selected_vm + "/disk/0_current.qcow2"),
					"-cdrom", cdrom_dir,
...
pub fn remove(vm_name: Option<&str>) {
	let selected_vm: &str 	= match vm_name {
		Some(name) => name,
		None => &terminal::input("Selected Virtual Machine Name > "),};
	fs::remove_dir_all(data::get_data_dir() + "/" + selected_vm)
...
pub fn rename(vm_name: Option<&str>) {
	...
	fs::rename(data::get_data_dir() + "/" + selected_vm, data::get_data_dir() + "/" + new_name)
...
pub fn create_event(vm_name: Option<&str>) {
	...
	let timestamps: Vec<OsString> = fs::read_dir(data::get_data_dir() + "/" + selected_vm + "/disk").unwrap()
	...
	fs::rename(data::get_data_dir() + "/" + selected_vm + "/disk/0_current.qcow2", data::get_data_dir() + "/" + selected_vm + "/disk/" + event_file_name)
		.expect("Failed to rename current state");

	Command::new("qemu-img").args(["create",
			"-f", "qcow2",
			"-b", &(data::get_data_dir() + "/" + selected_vm + "/disk/" + event_file_name),
			"-F", "qcow2", &(data::get_data_dir() + "/" + selected_vm + "/disk/0_current.qcow2")
		])
		.status().unwrap();
}
...
pub fn revert_back(vm_name: Option<&str>) {
	...
	if timestamps.len() != 1 {
		fs::remove_file(data::get_data_dir() + "/" + selected_vm + "/disk/0_current.qcow2")
			.expect("Failed to remove current state");
		fs::rename(data::get_data_dir() + "/" + selected_vm + "/disk/" + most_current_event, data::get_data_dir() + "/" + selected_vm + "/disk/0_current.qcow2")
...
pub fn show_timeline(vm_name: Option<&str>) {
	...
	for event_file in fs::read_dir(data::get_data_dir() + "/" + selected_vm + "/disk/").unwrap() {
